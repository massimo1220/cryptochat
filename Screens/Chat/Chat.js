...
export default class Chat extends Component {
  ...
  updateNewMessageCount = async (messages) => {
    ...
    messages.forEach((msg) => {
      const existingCounter = newMessages.find(
        (counter) => counter.from === msg.from
      );
      if (existingCounter) {
  ...
  getMessages = async (user) => {
    try {
      ...
      const { email } = user;
      const inboxSnap = await firebase
        .firestore()
        .collection("users")
        .doc(email)
        .collection("inbox")
      ...
      if (
        (builtMessages.length > 0 && this.state.conversations.length === 0) ||
        inbox.length
      ) {
      ...
      this.setState({
        updating: false,
        error:
          "Si è verificato un problema durante il recupero dei nuovi messaggi",
      });
    }
  };
  ...
  decryptMessages = async (messages) => {
    try {
      ...
      this.setState({
        error: "There was a problem decrypting your new message(s)",
      });
    }
  };
  ...
  buildMessages = async () => {
    try {
      ...
      const stringySavedMessages = await SecureStore.getItemAsync(
        user.email.replace("@", "")
      );
      ...
      this.setState({
        error:
          "There was a problem displaying your messages, please restart the application",
      });
    }
  };
  ...
  buildConversations = (messages) => {
    ...
    const sortedMessages = messages.reduce((conversations, curMessage) => {
      ...
      if (existingConvo > -1 && curMessage.from !== undefined) {
        const newMessage = {
          contents: curMessage.contents,
          timestamp: curMessage.sent,
        };
  ...
  saveNewMessages = async (messages) => {
    try {
      ...
      this.setState({
        error: "There was a problem when trying to save a new message",
      });
    }
  };
  ...
  renderConversationTabs = () => {
    ...
    return conversations.map((convo) => {
      const newMessagesCount = this.getConvoNewMessageCount(
        convo.from,
        newMessages
      );
      let time;
  ...
  getConvoNewMessageCount = (friend, newMessages) => {
    const index = newMessages.findIndex(
      (newMessage) => newMessage.from == friend
    );
    if (index > -1) {
  ...
  deleteConversation = async () => {
    ...
    const updatedConversations = conversations.filter(
      (convo) => convo.from !== toDeleteFrom
    );
    let storedMessages = await SecureStore.getItemAsync(
      user.email.replace("@", "")
    );
    storedMessages = JSON.parse(storedMessages);
    storedMessages.inbox = storedMessages.inbox.filter(
      (msg) => msg.from !== toDeleteFrom
    );
    storedMessages.sent = storedMessages.sent.filter(
      (msg) => msg.to !== toDeleteFrom
    );
    await SecureStore.setItemAsync(
      user.email.replace("@", ""),
  ...
  deleteMessage = async (from, contents, isSender) => {
    ...
    this.setState({ conversations });
    let savedMessages = await SecureStore.getItemAsync(
      user.email.replace("@", "")
    );
    savedMessages = JSON.parse(savedMessages);
    if (isSender) {
      savedMessages.sent = savedMessages.sent.filter(
        (msg) => msg.contents !== contents
      );
    } else {
      savedMessages.inbox = savedMessages.inbox.filter(
        (msg) => msg.contents !== contents
      );
    }
    await SecureStore.setItemAsync(
      ...
      <View style={styles.container}>
        ...
        {conversations.length === 0 && !loadingFonts && !loadingMessages && (
          <View style={styles.noMessagesContainer}>
            ...
            <Text style={styles.subNoMessagesText}>
              Premi il tasto aggiungi per iniziare una chat
            </Text>
          </View>
        )}
